...
// Stop the Motor
uint32_t stop_state_enter_time;
// To test queuing function, we will add additional states that will not exit into wait state immediatly, but will have some delay before going back to wait
void sm_state_stop(void) {
  if (lastState != smState) {
#if DEBUG_STATES == true
    Serial.println(F("state: STOP"));
#endif
    stop_state_enter_time = millis();

    lastState = smState;
  }
  //smState = STATE_STOPSTREAM;   Removed for test purposes
  if (millis() - stop_state_enter_time >= 30000) {
    smState = STATE_WAIT;
  }
}
